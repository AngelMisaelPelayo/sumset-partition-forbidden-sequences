...
|---|---|---|
| `verify.py` | §2, §3.1 | Brute-force reference solver; exhaustively verifies heredity and the singleton rule on small ground sets. |
| `solver.py` | §3, App. A | Fast exact realizability solver (singleton stripping + core packing) and the Hall-type necessary conditions. |
| `core_dp.py` | §3 (Prop. `coredp`) | Parameterized dynamic program for the core packing problem. |
| `experiments.py` | §3 | Validates `solver.py` against the brute force and runs the exhaustive classification experiments. |
| `pairs_scan.py` | §3.4 | Counterexample search verifying the pairs conjecture for all sequences with `m_1 ≤ 18`. |
| `minimal_catalog.py` | §4 | First-repeat reduction and the catalog of all minimal forbidden sequences with `m_1 ≤ 12`. |
| `check_c3.py` | §4.4 | Verifies that no minimal forbidden sequence lies on the exact-partition boundary `Σ m_i = m_1(m_1+1)/2`. |
| `nreal_tree.py` | §5, App. B | Greedy assignment tree for exact n-realizability; computes the order of non-realizable feasible sequences and the census for `n ≤ 12`. |
| `c4.py` | §6 | Order-hierarchy results: second-entry bound, step bound, the floor-and-towers family, and `maxord(n) = n − 5` for `8 ≤ n ≤ 31`. |
...
```sh
python3 check_c3.py
python3 minimal_catalog.py
```

The longer exhaustive runs (`pairs_scan.py` at `m_1 = 18`, `c4.py` at
